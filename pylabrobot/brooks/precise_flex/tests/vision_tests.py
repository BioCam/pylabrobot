...
class TestVisionSetupGating(unittest.IsolatedAsyncioTestCase):
  ...
  def _arm(self, has_vision_module: bool) -> PreciseFlex:
    ...
    arm.set_response_mode = AsyncMock()  # type: ignore[method-assign]
    ...
    arm._handle_out_of_range_axes = AsyncMock()  # type: ignore[method-assign]
  ...
  async def test_vision_connected_when_module_present(self):
    ...
    with patch.object(
      arm, "_setup_vision", AsyncMock(side_effect=fake_setup_vision)
    ) as setup_vision:
      await arm.setup()
    setup_vision.assert_awaited_once_with(arm._vision_host)
...
def _controller_with_io() -> "tuple[PreciseFlex, MagicMock]":
  ...
  arm.io = io  # type: ignore[assignment]
  return arm, io
