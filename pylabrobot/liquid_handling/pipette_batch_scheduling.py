...
def _pairwise_min_spacing(spacings: List[float], i: int, j: int) -> float:
  ...
  for batch_idx, batch in enumerate(batches):
    ...
    for ch in range(num_channels):
      ...
      if prev_positions is not None and ch in prev_positions:
        ...
        spacing = _pairwise_min_spacing(spacings, 0, 1)
        positions[ch] = positions.get(1, max_y) + spacing
      else:
        spacing = _pairwise_min_spacing(spacings, ch - 1, ch)
    ...
    for ch in range(num_channels - 2, -1, -1):
      if ch in fixed:
        continue
      spacing = _pairwise_min_spacing(spacings, ch, ch + 1)
      if positions[ch] - positions[ch + 1] < spacing - 1e-9:
        positions[ch] = positions[ch + 1] + spacing

    # 5. Bounds clamp (free channels only).
    if 0 not in fixed and positions[0] > max_y:
      positions[0] = max_y
    if (num_channels - 1) not in fixed and positions[num_channels - 1] < min_y:
      positions[num_channels - 1] = min_y
...
def validate_probing_inputs(
  ...
  """Validate and normalize inputs for liquid height probing.
