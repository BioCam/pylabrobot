...
class TestPairwiseMinSpacing(unittest.TestCase):
  def test_uniform_spacing(self):
    spacings = [9.0] * 8
    self.assertAlmostEqual(_pairwise_min_spacing(spacings, 0, 1), 9.0)
    self.assertAlmostEqual(_pairwise_min_spacing(spacings, 5, 6), 9.0)
  ...
  def test_mixed_spacing(self):
    ...
    self.assertAlmostEqual(_pairwise_min_spacing(spacings, 1, 2), 18.0)
    # max(17.96, 17.96) = 17.96 → ceil(179.6)/10 = 18.0
    self.assertAlmostEqual(_pairwise_min_spacing(spacings, 2, 3), 18.0)
...
class TestForwardPlan(unittest.TestCase):
  ...
  def test_no_future_use_channels_packed_tightly(self):
    ...
    for ch in range(2, self.N):
      spacing = _pairwise_min_spacing(self.S, ch - 1, ch)
