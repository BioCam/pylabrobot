...
async def probe_tip_inventory(
  ...
  *,
  ...
  """Probe any number of spots, as many at a time as there are channels given.
  ...
  Raises:
    ValueError: If no channel is given, or a channel is given twice.
  """
  if not use_channels:
    raise ValueError("use_channels must not be empty")
  present: Dict[str, bool] = {}
  for start in range(0, len(tip_spots), len(use_channels)):
    chunk = tip_spots[start : start + len(use_channels)]
    present.update(
      await probe_tip_presence_via_pickup(
        chunk,
        use_channels[: len(chunk)],
