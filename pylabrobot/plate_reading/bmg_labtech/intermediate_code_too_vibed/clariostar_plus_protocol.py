...
def decode_cmd05(ann, payload, off):
  ...
  sub_labels = {
    ...
    0x09: "FIRMWARE_INFO",
    0x0F: "FOCUS_HEIGHT",
    0x17: "UNKNOWN_0x17 (pre-measurement query?)",
    0x1D: "READ_ORDER",
    0x21: "USAGE_COUNTERS",
  }
...
def _decode_abs_post_separator(ann, post, po):
  ...
  for w in range(num_wl):
    ...
    ann.add(
      po + idx, po + idx + 2, f"Wavelength {w + 1}: {wl_raw / 10:.0f} nm (raw=0x{wl_raw:04x})"
    )
    idx += 2

  if idx + 13 > len(post):
    if idx < len(post):
      ann.add(po + idx, po + len(post), f"[truncated reference block: {post[idx:].hex(' ')}]")
    return
  ...
  if ref_hi:
    ann.add(
      po + idx, po + idx + 2, f"Ref wavelength high: {ref_hi / 10:.0f} nm (raw=0x{ref_hi:04x})"
    )
  else:
  ...
  if ref_lo:
    ann.add(
      po + idx, po + idx + 2, f"Ref wavelength low: {ref_lo / 10:.0f} nm (raw=0x{ref_lo:04x})"
    )
  else:
  ...
  if idx + 3 > len(post):
    if idx < len(post):
      ann.add(po + idx, po + len(post), f"[truncated settling: {post[idx:].hex(' ')}]")
    return

  settling_flag = post[idx]
  ann.add1(
    po + idx, f"Settling flag: {'ENABLED' if settling_flag else 'disabled'} (0x{settling_flag:02x})"
  )
  idx += 1
  ...
  if idx + 11 > len(post):
    if idx < len(post):
      ann.add(po + idx, po + len(post), f"[truncated trailer: {post[idx:].hex(' ')}]")
    return

  ann.add(po + idx, po + idx + 11, f"Fixed trailer (11 bytes): {post[idx : idx + 11].hex(' ')}")
  idx += 11
...
def decode_data_response(ann, payload, off):
  ...
  if is_abs and len(payload) >= 36:
    ...
    ann.add(
      off + 23,
      off + 25,
      f"Temperature (offset 23): {temp23 / 10:.1f} C"
      if temp23 >= 50
    ...
    ann.add(
      off + 34,
      off + 36,
      f"Temperature (offset 34): {temp34 / 10:.1f} C"
      if temp34 >= 50
    ...
    if remaining > 0:
      ann.add(
        off + after_data, off + len(payload), f"Reference channels + calibration: {remaining} bytes"
      )
    ...
    ann.add(
      off + 25,
      off + 27,
      f"Temperature: {temp_raw / 10:.1f} C"
      if temp_raw >= 50
    ...
    if remaining > 0:
      ann.add(
        off + data_start + data_bytes, off + len(payload), f"[trailing bytes] {remaining} bytes"
      )
  else:
    ann.add(
      off + 11, off + len(payload), f"[data payload: {len(payload) - 11} bytes, not fully decoded]"
    )
...
def decode_eeprom_response(ann, payload, off):
  ...
  if len(payload) > 15:
    ann.add(off + 15, off + len(payload), f"[EEPROM data continuation: {len(payload) - 15} bytes]")
...
def decode_firmware_response(ann, payload, off):
  ...
  if len(payload) >= 8:
    ...
    ann.add(off + 6, off + 8, f"Firmware version: {version_raw / 1000:.2f} (raw={version_raw})")
  if len(payload) >= 20:
  ...
  if len(payload) > 28:
    ann.add(off + 28, off + len(payload), f"[unknown trailing: {payload[28:].hex(' ')}]")
...
def decode_read_order_response(ann, payload, off):
  ...
  if len(payload) >= 19:
    ...
    if len(data) >= n_entries * 2:
      ...
      ann.add(off + 19, off + 19 + n_entries * 2, f"Read order (col,row 1-based): {summary}")
...
def decode_frame(direction: str, raw: bytes) -> Annotation:
  ...
  if direction == "SEND":
    if cmd == 0x01:
      ...
      ann.add(off, off + len(payload), f"[unknown command 0x{cmd:02x}, {len(payload)} bytes]")
  else:  # RECV
    if cmd == 0x01:
      ...
      payload[i] == 0 or any(payload[bi] & m for bi, m, _ in STATUS_FLAGS if bi == i)
      for i in range(min(5, len(payload)))
    ):
      decode_status_response(ann, payload, off)
    else:
      ann.add(
        off, off + len(payload), f"[unknown response, first byte 0x{cmd:02x}, {len(payload)} bytes]"
      )
...
def write_frames_txt(
  frames: List[Tuple[float, str, bytes]], output_path: str, source_name: str = ""
):
...
def main():
  usage = f"Usage: {sys.argv[0]} <command> <path>\nCommands: extract, decode, both"
  if len(sys.argv) != 3:
    print(usage)
    sys.exit(1)
