...
observed in USB captures from real CLARIOstar Plus hardware.
Phase 4 adds absorbance measurement tests verified against hardware-verified ground truth.
"""
...
COMMANDS: Dict[str, tuple] = {
  ...
  # CF.TEMPERATURE(0x06), no command byte. K01 hardware-verified ground truth.
  "temp_off": (
    b"\x06\x00\x00",
  ...
  # CF.REQUEST(0x05) + Cmd.DATA(0x02) — standard variant. Capture: A01 final frame.
  "get_data_standard": (
    b"\x05\x02\x00\x00\x00\x00\x00",
    bytes.fromhex("02000f0c050200000000000000240d"),
  ),
  # CF.REQUEST(0x05) + Cmd.DATA(0x02) — progressive variant. Capture: A01 mid-measurement.
  "get_data_progressive": (
    b"\x05\x02\xff\xff\xff\xff\x00",
    bytes.fromhex("02000f0c0502ffffffff000004200d"),
  ),
  # CF.REQUEST(0x05) + Cmd.READ_ORDER(0x1D). Capture: all measurement captures.
  "read_order": (
    b"\x05\x1d\x00\x00\x00\x00\x00",
    bytes.fromhex("02000f0c051d000000000000003f0d"),
  ),
  # CF.PAUSE_RESUME(0x0D) — pause. Capture: stop_and_abandon capture @17.3s.
  "pause_measurement": (
    b"\x0d\xff\xff\x00\x00",
    bytes.fromhex("02000d0c0dffff00000002260d"),
  ),
  # CF.PAUSE_RESUME(0x0D) — resume. Capture: stop_and_abandon capture @78.4s.
  "resume_measurement": (
    b"\x0d\x00\x00\x00\x00",
    bytes.fromhex("02000d0c0d000000000000280d"),
  ),
  # CF.STOP(0x0B). Capture: stop_and_abandon capture @86.0s.
  "stop_measurement": (
    b"\x0b\x00",
    bytes.fromhex("02000a0c0b000000230d"),
  ),
  # CF.CMD_0x0E(0x0E). Capture: normal_power_cycle_oem_start capture @34.3s.
  "cmd_0x0e": (
...
def _make_frame_test(name, payload, expected_frame):
  ...
  test.__doc__ = f"_wrap_payload must match hardware-verified ground truth for {name!r}"
  return test
...
class TestInitialize(unittest.TestCase):
  """Hardware-verified ground truth for initialize command and its status response."""
...
class TestSetupSendsCmdOx0e(unittest.TestCase):
  """Verify that setup() sends CMD_0x0E after EEPROM + firmware reads (matches OEM capture)."""
...
class TestOpen(unittest.TestCase):
  """Hardware-verified ground truth for drawer-open command and its status response."""
...
class TestClose(unittest.TestCase):
  """Hardware-verified ground truth for drawer-close command and its status response."""
...
class TestStatusParsing(unittest.TestCase):
  ...
  def test_reading_wells_with_busy_and_initialized(self):
    # Real capturepattern: busy (byte 1: 0x20) + initialized + reading_wells (byte 3: 0x28)
...
class TestTemperature(unittest.TestCase):
  """Verify temperature commands use standard framing (K01 hardware-verified ground truth)."""
...
def _build_synthetic_response(
  ...
  Real device behavior is validated by test_D02/D03 ground truth tests.
...
class TestPlateField(unittest.TestCase):
  """Verify _plate_field well mask encoding against hardware-verified ground truth."""
  ...
  def test_column1_mask(self):
    """Column 1 (A1,B1,...,H1) → matches capture80 08 00 80 08 00 80 08 00 80 08 00."""
...
class TestScanDirectionByte(unittest.TestCase):
  """Verify _scan_direction_byte against all 6 known capturevalues."""

  def test_uni_tl_vert(self):
    self.assertEqual(CLARIOstarPlusBackend._scan_direction_byte(False, True, "TL"), 0x8A)

  def test_bidi_tl_vert(self):
    self.assertEqual(CLARIOstarPlusBackend._scan_direction_byte(True, True, "TL"), 0x0A)

  def test_tr(self):
    self.assertEqual(CLARIOstarPlusBackend._scan_direction_byte(True, True, "TR"), 0x2A)

  def test_bl(self):
    self.assertEqual(CLARIOstarPlusBackend._scan_direction_byte(True, True, "BL"), 0x4A)

  def test_br(self):
    self.assertEqual(CLARIOstarPlusBackend._scan_direction_byte(True, True, "BR"), 0x6A)

  def test_horizontal(self):
    self.assertEqual(CLARIOstarPlusBackend._scan_direction_byte(True, False, "TL"), 0x02)
...
class TestMeasurementData(unittest.TestCase):
  ...
  def test_request_measurement_data_standard_payload(self):
    """Standard variant sends 05 02 00 00 00 00 00 (7 bytes, matching capture)."""
    backend = _make_backend()
    mock: MockFTDI = backend.io  # type: ignore[assignment]
  ...
  def test_measurement_progress_ground_truth(self):
    """Verify _measurement_progress against real captureA01 progressive response header.
    ...
    """
    # First 36 bytes of the progressive response inner payload from USB capture
    captured_header = bytes.fromhex(
      "0225042e0000a901880000001dffe2022003000100600100010000000101000000000000"
    )
    written, expected = CLARIOstarPlusBackend._measurement_progress(captured_header)
  ...
  def test_measurement_progress_too_short_raises(self):
    ...
    from pylabrobot.plate_reading.bmg_labtech.clariostar_plus import FrameError
...
class TestBuildAbsorbancePayload(unittest.TestCase):
  """Verify _build_absorbance_payload against hardware-verified ground truth.
...
class TestBuildAbsorbanceSpectrumPayload(unittest.TestCase):
  """Verify _build_absorbance_spectrum_payload against hardware-verified ground truth.
  ...
  def test_H01_spectrum_300_700_1nm(self):
    ...
    OEM software uses bidirectional scanning for spectrum mode (scan byte 0x0A).
    """
...
# ---------------------------------------------------------------------------
# Spectrum ground truth tests — real USB capture data
# ---------------------------------------------------------------------------

# OEM software OD at 600nm for all 96 wells from the verification plate spectrum scan
...
# OEM software OD at selected wavelengths for A01 from the verification plate spectrum scan.
...
# OEM software OD at selected wavelengths for B01 (low-OD clear well).
...
def _load_test_data_pages(prefix: str) -> List[bytes]:
  ...
  Args:
    prefix: File prefix (e.g., 'abs_spectrum_300_700_1nm_point_96w').
...
class TestSpectrumResponseGroundTruth(unittest.TestCase):
  """Ground truth tests using real capturespectrum response data.

  Binary page payloads were extracted from USB USB captures of real
  ...
  4. Verification plate OD matches OEM software ground truth CSV
  ...
  def test_h01_page_structure(self):
    ...
    pages = _load_test_data_pages("abs_spectrum_300_700_1nm_point_96w")
  ...
  def test_h02_page_structure(self):
    ...
    pages = _load_test_data_pages("abs_spectrum_400_600_1nm_point_96w")
  ...
  def test_h03_page_structure(self):
    ...
    pages = _load_test_data_pages("abs_spectrum_300_700_5nm_point_96w")
  ...
  def test_h04_page_structure(self):
    ...
    H04 was captured with 1nm step (confirmed from captureRUN payload),
    giving 401 wavelengths. Values: (401+3)×(8+2) = 4040.
    """
    pages = _load_test_data_pages("abs_spectrum_300_700_1nm_point_8w")
  ...
  def test_h05_page_structure(self):
    ...
    pages = _load_test_data_pages("abs_spectrum_300_700_1nm_orbital_96w")
    self._assert_page_headers(pages, expected_num_wells=96, expected_page_count=3)
    self.assertEqual(self._total_values(pages), 39592)  # (401+3)×(96+2)
  ...
  def test_verification_page_structure(self):
    ...
    pages = _load_test_data_pages("abs_spectrum_220_1000_1nm_spiral_96w")
  ...
  def test_verification_od_600nm_all_96_wells(self):
    """Verification: OD at 600nm matches OEM software ground truth for all 96 wells.
  ...
  def test_verification_a01_multi_wavelength(self):
    """Verification: A01 OD at multiple wavelengths matches OEM software ground truth.
  ...
  def test_h01_h05_same_wavelengths_different_scan(self):
    ...
    """
    h01_pages = _load_test_data_pages("abs_spectrum_300_700_1nm_point_96w")
    h05_pages = _load_test_data_pages("abs_spectrum_300_700_1nm_orbital_96w")
    wavelengths = list(range(300, 701))
  ...
  def test_h01_h03_overlapping_wavelengths(self):
    ...
    """
    h01_pages = _load_test_data_pages("abs_spectrum_300_700_1nm_point_96w")
    h03_pages = _load_test_data_pages("abs_spectrum_300_700_5nm_point_96w")
...
class TestParseAbsorbanceResponse(unittest.TestCase):
  ...
  def test_A05_single_well_ground_truth(self):
    """Verify OD against real captureA05 capture (single well A1, 600nm, point scan).

    Source: A05_A1_point_frames.txt — final standard DATA response (84-byte payload).
    Expected OD ≈ 0.0771 (blank/clear well).
    """
    # Inner payload from captureA05 final standard DATA response (wire[4:-4])
  ...
  def test_A03_col1_dilution_ground_truth(self):
    """Verify OD against real captureA03 capture (col 1, 8 wells, 600nm, point scan).
    ...
    """
    # Inner payload from captureA03 final standard DATA response (wire[4:-4])
    payload = bytes.fromhex(
    ...
    # Expected OD values for column 1 dilution series (from capturecomputation)
  ...
  def test_D02_dual_wavelength_ground_truth(self):
    """Verify dual-WL OD against real captureD02 capture (col 1, 8 wells, 450+600nm).
    ...
    """
    # 8-well dual-WL payload built from real D02 captureA1-H1 values
    payload = bytes.fromhex(
  ...
  def test_D03_triple_wavelength_ground_truth(self):
    """Verify triple-WL OD against real captureD03 capture (col 1, 8 wells, 450+600+660nm).

    Source: D03_triple_frames.txt — final standard DATA response.
    Layout: group0(WL1) + WL2 + WL3 + chrom2 + chrom3 + ref, with 6 cal pairs.
    """
    # 8-well triple-WL payload built from real D03 captureA1-H1 values
    payload = bytes.fromhex(
    ...
    # Expected ODs from capturecomputation
...
class TestReadAbsorbanceIntegration(unittest.TestCase):
  ...
  def test_request_absorbance_results(self):
    ...
    # Verify it sent the standard (non-progressive) payload (7 bytes, matching capture)
    inner = _extract_payload(mock.written[0])
    self.assertEqual(inner, b"\x05\x02\x00\x00\x00\x00\x00")
...
class TestSendCommandFrameRetry(unittest.TestCase):
  ...
  def test_retries_on_truncated_frame(self):
    ...
    # Truncated 18 bytes of a 53-byte DRAWER_CLOSE response (hardware-verified ground truth
...
# ===========================================================================
# Ground truth hex from 20 USB captures (F-A01 through F-L01).
...
# FL DATA_RESPONSE hex (full wire frames, exact from capture)
_FL_RESP_HEX = {
...
class TestBuildFluorescencePayload(unittest.TestCase):
  ...
  def test_FA01_baseline_top_point(self):
    ...
    payload = self.backend._build_fluorescence_payload(
      ...
      settling_time_s=0.1,
      well_scan="point",
      # Exact capturewavelength values (bypass ±2-3 firmware calibration)
  ...
  def test_nominal_wavelength_encoding(self):
    """Verify nominal wavelength encoding without captureoverrides."""
    payload = self.backend._build_fluorescence_payload(
      self.plate,
      self.all_wells,
      excitation_wavelength=485,
...
class TestFluorescenceFrameValidity(unittest.TestCase):
  """Verify all new FL hardware-verified ground truth frames have valid checksums."""
...
class TestFluorescencePostSeparatorGroundTruth(unittest.TestCase):
  """Byte-level verification of post-separator fields from new USB captures.
...
class TestParseFluorescenceResponse(unittest.TestCase):
  """Verify _parse_fluorescence_response against hardware-verified ground truth."""
...
class TestBuildFluorescencePayloadFlying(unittest.TestCase):
  ...
  def test_flying_matches_FO01_ground_truth(self):
    """Flying payload matches F-O01 capturepost-separator fields."""
    payload = self.backend._build_fluorescence_payload(
      self.plate, self.all_wells,
      excitation_wavelength=485, emission_wavelength=528, focal_height=8.5,
      flying_mode=True,
...
class TestBuildFluorescencePayloadFilter(unittest.TestCase):
  ...
  def test_all_filter_matches_FLf01(self):
    """All-filter payload matches F-Lf01 capturepost-separator."""
    OpticalFilter = CLARIOstarPlusBackend.OpticalFilter
    payload = self.backend._build_fluorescence_payload(
      self.plate, self.all_wells,
      excitation_wavelength=485, emission_wavelength=528, focal_height=8.5,
...
class TestBuildFluorescencePayloadMatrix(unittest.TestCase):
  ...
  def test_matrix_matches_FS01_post_separator(self):
    """Matrix payload matches F-S01 capturepost-separator."""
    wells = [self.plate.get_item("A1")]
    payload = self.backend._build_fluorescence_payload(
      self.plate, wells,
      excitation_wavelength=485, emission_wavelength=528, focal_height=8.5,
...
class TestBuildFluorescencePayloadDualChromatic(unittest.TestCase):
  ...
  def test_dual_chromatic_matches_FM01(self):
    """Dual chromatic payload matches F-M01 capture."""
    payload = self.backend._build_fluorescence_payload(
      self.plate, self.all_wells,
      excitation_wavelength=485, emission_wavelength=528, focal_height=8.5,
      chromatics=[
...
# Ground truth from F-Q01 USB capture.
...
# FOCUS_RESULT (0x05/0x05) RECV — full inner payload from F-Q01 capture(1177 bytes).
...
class TestBuildAutofocusPayload(unittest.TestCase):
  """Tests for _build_autofocus_payload against F-Q01 hardware-verified ground truth."""
  ...
  def test_post_plate_structure_matches_ground_truth(self):
    """Structural bytes (non-wavelength) should match F-Q01 hardware-verified ground truth."""
    backend = _make_backend()
    plate = _make_plate()
    wells = plate.get_all_items()
    payload = backend._build_autofocus_payload(
  ...
  def test_wavelengths_within_calibration_tolerance(self):
    """Wavelength edge values should be within ±3 of capture(monochromator calibration)."""
    backend = _make_backend()
    plate = _make_plate()
    wells = plate.get_all_items()
    payload = backend._build_autofocus_payload(
    ...
    # capturevalues: ExHi=0x133b=4923, ExLo=0x12a9=4777, Dich=0x13bc=5052,
    #              EmHi=0x1501=5377, EmLo=0x143f=5183
    captured_wl = [4923, 4777, 5052, 5377, 5183]
    for i, expected in enumerate(captured_wl):
      actual = int.from_bytes(payload[74 + i*2 : 76 + i*2], "big")
      self.assertAlmostEqual(actual, expected, delta=3,
        msg=f"Wavelength pair {i} at offset {74+i*2}: {actual} vs captured {expected}")
...
class TestParseFocusResult(unittest.TestCase):
  """Tests for _parse_focus_result against full F-Q01 hardware-verified ground truth (1177B payload)."""
...
class TestAutoFocusIntegration(unittest.TestCase):
  ...
  def test_auto_focus_returns_focal_height(self):
    """auto_focus with real capturepayload should return 10.0mm and 143 records."""
    backend = _make_backend()
    mock: MockFTDI = backend.io  # type: ignore[assignment]
    plate = _make_plate()
    wells = plate.get_all_items()
    ...
    #  (3) real capturefocus result for _request_focus_result()
    captured_result = bytes.fromhex(_AF_GT_RESULT_FULL)
    mock.queue_response(
      ACK, self._build_idle_status(),
      _wrap_payload(captured_result),
    )
...
class TestFilterDetection(unittest.TestCase):
  ...
  Ground truth from clariostar_plus_filter_autodetection_routine USB capture.
  ...
  def test_build_filter_scan_payload_excitation(self):
    ...
    # Capture cycle 1 frame (after STX/size/0x0C, before checksum/CR):
    ...
    # Verify the full command frame matches capture
  ...
  def test_build_filter_scan_payload_emission(self):
    ...
    self.assertEqual(params, expected)
    # Verify full frame matches capturecycle 8
  ...
  def test_build_filter_scan_payload_dichroic(self):
    ...
    self.assertEqual(params, expected)
    # Verify full frame matches capturecycle 7
  ...
  def test_parse_filter_result_excitation_occupied(self):
    """Capture cycle 1: Ex pos 1 → BP 327/90 (Ex TR filter installed).
  ...
  def test_parse_filter_result_emission_occupied(self):
    """Capture cycle 8: Em pos 5 → BP 614/18 (615-18 filter installed).
  ...
  def test_parse_filter_result_dichroic_occupied(self):
    """Capture cycle 5: Dichroic A → DM 422 (LP TR filter installed).
  ...
  def test_parse_filter_result_empty(self):
    """Capture cycle 2: Ex pos 2 → empty (bandwidth = 0).
  ...
  def test_parse_filter_result_dichroic_empty(self):
    """Capture cycle 6: Dichroic B → empty (cut-on = 0)."""
    payload = bytes([
      0x1b, 0x05, 0x13, 0x26, 0x80, 0x00,  # header
      0x83,                                   # type: dichroic
      0x00, 0x00,                             # no center
  ...
  def test_detect_all_filters(self):
    ...
    Expected results (matching capture+ OEM software screenshots):
...
# Hardware-verified ground truth: excitation scan F-SCAN-A01 post-separator (68 bytes)
...
# Hardware-verified ground truth: emission scan F-SCAN-A02 post-separator (68 bytes)
...
class TestBuildFluorescenceSpectrumPayload(unittest.TestCase):
  """Verify _build_fl_spectrum_payload against hardware-verified ground truth."""
  ...
  def test_excitation_scan_post_separator_ground_truth(self):
    """Post-separator bytes must match F-SCAN-A01 captureexactly."""
  ...
  def test_emission_scan_post_separator_ground_truth(self):
    """Post-separator bytes must match F-SCAN-A02 captureexactly."""
    payload = self.backend._build_fl_spectrum_payload(
      self.plate, self.all_wells,
...
class TestMeasurementControl(unittest.TestCase):
  """Hardware-verified ground truth for pause_measurement, resume_measurement, stop_measurement.

  Source: clariostar_stop_then_abort_drawer_in_out USB capture
  """
  ...
  def test_pause_payload_matches_ground_truth(self):
    """Byte-for-byte match against captureframe at t=17.3s."""
    expected = bytes.fromhex("02000d0c0dffff00000002260d")
    self.assertEqual(_wrap_payload(b"\x0d\xff\xff\x00\x00"), expected)

  def test_resume_payload_matches_ground_truth(self):
    """Byte-for-byte match against captureframe at t=78.4s."""
    expected = bytes.fromhex("02000d0c0d000000000000280d")
    self.assertEqual(_wrap_payload(b"\x0d\x00\x00\x00\x00"), expected)

  def test_stop_payload_matches_ground_truth(self):
    """Byte-for-byte match against captureframe at t=86.0s."""
  ...
  def test_cmd_0x0e_payload_matches_ground_truth(self):
    """Byte-for-byte match against captureframe from normal boot capture."""
...
# ---------------------------------------------------------------------------
# Standalone Shaking — hardware-verified ground truth tests
# ---------------------------------------------------------------------------
...
class TestStartShaking(unittest.TestCase):
  """Verify start_shaking() payload bytes against DDE hardware-verified ground truth.

  Ground truth from SH-01 through SH-10 captures.
  Captured payload (11 bytes, no command sub-byte):
    [0x1D] [mode] [speed_idx] [duration:2B BE] [x:2B BE] [y:2B BE] [0x00] [flags]

  Tests verify the PAYLOAD matches captureexactly. Our framing (3-byte checksum)
  differs from OEM software framing (1-byte checksum), but the device accepts both.
  """

  def test_firmware_ground_truth(self):
    for name, capture, pattern, rpm, dur, x, y, expected_hex in _SHAKING_GT_CASES:
...
class TestStopShaking(unittest.TestCase):
  ...
  ST-01/ST-04 hardware-verified ground truth: payload = 0b 00
  """
...
# ---------------------------------------------------------------------------
# Idle Movement — hardware-verified ground truth tests
# ---------------------------------------------------------------------------
...
class TestStartIdleMovement(unittest.TestCase):
  """Verify start_idle_movement() payload bytes against DDE hardware-verified ground truth.
  ...
  def test_firmware_ground_truth(self):
    for name, capture, pattern, dur, on_t, off_t, expected_hex in _IDLE_MOVE_GT_CASES:
      with self.subTest(name=name, capture=capture):
        backend, mock = _make_backend_and_mock()
        mock.queue_response(ACK)
...
class TestStopIdleMovement(unittest.TestCase):
  ...
  IM-03/IM-05 capturepayload: 27 00 00 00 00 00 00 00 00 00 00
  """

  def test_cancel_payload(self):
    """IM-03: cancel idle movement (mode=0, all zeros)."""
    backend, mock = _make_backend_and_mock()
    mock.queue_response(ACK)
