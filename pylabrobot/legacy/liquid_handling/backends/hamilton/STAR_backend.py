...
from pylabrobot.hamilton.protocol.text.framing import (
  parse_fw_string as parse_star_fw_string,  # noqa: F401
)
...
class STARBackend(HamiltonLiquidHandler, HamiltonHeaterShakerInterface):
  ...
  def _parse_firmware_version_datetime(self, fw_version: str) -> datetime.date:
    ...
    Kept as a method for its callers; the parsing itself lives in the shared text-protocol
    framing module, so every Hamilton machine that speaks it reads versions the same way.
    """
    return parse_firmware_version_date(fw_version)
